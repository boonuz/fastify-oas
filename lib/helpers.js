  ...
  Object.keys(src).forEach((key) => {
    ...
    if (src[key].type || src[key].oneOf || src[key].$ref) {
      ...
      for (const mediaType of mediaTypes) {
        ...
        if (rsp.examples) {
          dst[key].content[mediaType].examples = rsp.examples.reduce(
            (res, { name, ...rest }) => {
              res[name] = rest;
              return res;
            },
          ...
          delete dst[key].content[mediaType].schema.examples;
          delete rsp.examples;
        } else if (rsp.example) {
          dst[key].content[mediaType].example = rsp.example;
          delete dst[key].content[mediaType].schema.example;
          delete rsp.example;
        }
      }
    }
